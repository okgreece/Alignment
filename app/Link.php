...
class Link extends Model {
    ...
    public static function existing(Request $request) {
        return $link = Link::where('project_id', '=', $request->project_id)
    ...
    public function getConfidence(Link $link) {
    ...
    public static function downloadFile($file, $name, $format) { // $file = include path
        if (file_exists($file)) {
    ...
    public static function exportFile($myGraph, $format, $project_id) {
        ...
        $file = Link::filename($format, $project_id);
        file_put_contents($file["path"], $export);
        Link::downLoadFile($file["path"], $file["filename"], $format);
    }
}
